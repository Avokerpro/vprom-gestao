...
import { Lock, Mail, Loader2, ShieldAlert, Fingerprint, ShieldCheck } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onOfflineLogin }) => {
            ...
            <form onSubmit={handleManualLogin} className="space-y-6">
                ...
                <div className="flex items-center gap-2 mt-4 px-1">
                  <input 
                    type="checkbox" 
                    id="remember" 
                    className="w-4 h-4 accent-vprom-orange"
                    checked={rememberBiometrics}
                    onChange={(e) => setRememberBiometrics(e.target.checked)}
                  />
                  <label htmlFor="remember" className="text-[10px] font-black text-gray-400 uppercase">Lembrar neste dispositivo</label>
                </div>
            </form>
        </div>
      </div>
